...
if [ -f "${APP_DIR}/composer.json" ]; then
    ...
    # 2) Laravel app key (only if missing or malformed)
    if [ "${SKIP_KEY_GENERATE:-0}" != "1" ] && [ -f "artisan" ] && [ -f ".env" ]; then
        APP_KEY_LINE=$(grep -E '^APP_KEY=' .env | head -n 1 || true)
        NEED_KEY=0
        case "$APP_KEY_LINE" in
            ''|APP_KEY=|APP_KEY=null) NEED_KEY=1 ;;
            *)
                case "$APP_KEY_LINE" in
                    *base64:*base64:*) NEED_KEY=1 ;;
                esac
                ;;
        esac
